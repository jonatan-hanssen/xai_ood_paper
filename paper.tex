...
\end{tikzpicture}
    \caption{Figure showing the function of the SalAgg+MLS OOD detection framework. We here imagine an animal classifyier, and feed it two example images; one with an animal present (an ID data point), and one without any animals (an OOD data point). Given a model $f$ (e.g. ResNet18), a saliency mapping method $s$ (e.g. Guided Backpropagation) and an aggregation function $A$ (e.g. the vector norm), we calculate the saliency aggregate and the value of the highest logit, and combine their Z-scores to get a number indicating OOD-ness. In this case, we can imagine that the threshold $\delta$ is set at $1$, leading to the picture with an animal being classified as ID and the picture without an animal being classified as OOD.}
    \label{fig:salagg}
\end{figure}
