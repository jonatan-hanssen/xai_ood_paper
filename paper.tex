...
In this section, we introduce our framework for using \ac{xai} saliency methods for \ac{ood} detection. This framework aggregates the saliency values (heatmaps) outputted by methods such as GradCAM, GBP or LIME, and uses this value to determine \ac{ood}-ness. Given the large number of existing XAI saliency methods and possible aggregate functions (such as the mean, maximum, third quartile or median), the framework opens up for a range of new OOD detection methods. In addition to the information extracted by saliency aggregation, we also utilize the Maximum Logit score. As Shekhar et al. \cite{combood} have shown, \ac{sota} \ac{ood} detection performance can be achieved by combining different methods which extract information from the network in different ways. Inspired by this work, we present the SalAgg+MLS (Saliency Aggregation plus Maximum Logit Score) framework:
...
\node[box, label={[align=center]Classifier $f$}, right=of $(img1)!0.5!(img2)$, xshift=0cm] (model) {ResNet18};
...
\node[box, label={[align=center]{Saliency\\Method $s$}}, right=of model, xshift=-1.3cm] (saliency) {GradCAM};

\draw[-latex] ($(model.east)+ (0,-0.2)$) -> ($(saliency.west)+ (0,-0.2)$);
\draw[-latex] ($(model.east)+ (0,+0.2)$) -> ($(saliency.west)+ (0,+0.2)$);

\node[box, label={[align=center]ID image}, right=of img1, xshift=3.8cm] (img3) {\includegraphics[width=0.125\textwidth]{figure/meansaliency/meansaliency1.png}};
\node[box, label={[align=center]ID image}, right=of img2, xshift=3.8cm] (img4) {\includegraphics[width=0.125\textwidth]{figure/meansaliency/meansaliency3.png}};

\draw[-latex] ($(saliency.east)+ (0,+0.2)$) -- ++(0.3,0) |- (img3);
\draw[-latex] ($(saliency.east)+ (0,-0.2)$) -- ++(0.3,0) |- (img4);

\node[draw, minimum width=1.5cm, label={[align=center]ID MLS}, above=of img3, yshift=-0.7cm] (mls1) {3.4};
\node[draw, minimum width=1.5cm, label={[align=center]ID MLS}, below=of img4, yshift=0.7cm] (mls2) {1.2};

\draw[-latex] ($(model.east)+ (0,+0.4)$) -- ++(0.4,0) |- (mls1.west);
\draw[-latex] ($(model.east)+ (0,-0.4)$) -- ++(0.4,0) |- (mls2.west);

\node[box, label={[align=center]Aggregator $a$}, right=of saliency, xshift=1.4cm] (agg) {Mean};

\draw[-latex] (img3.east) -- ++(0.4,0) |- ($(agg.west)+ (0,+0.2)$);
\draw[-latex] (img4.east) -- ++(0.4,0) |- ($(agg.west)+ (0,-0.2)$);

\node[draw, minimum width=1.5cm, label={[align=center]ID Aggregate}, right=of agg, yshift=1cm, xshift=-1cm] (agg1) {1.4};
\node[draw, minimum width=1.5cm, label={[align=center]OOD Aggregate}, right=of agg, yshift=-1cm, xshift=-1cm] (agg2) {0.2};

\draw[-latex] ($(agg.east)+ (0,+0.4)$) -- ++(0.4,0) |- (agg1.west);
\draw[-latex] ($(agg.east)+ (0,-0.4)$) -- ++(0.4,0) |- (agg2.west);
